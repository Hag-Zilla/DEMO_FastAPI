...
def clear_response_cache() -> None:
    """Ensure FastAPICache is initialised (InMemoryBackend) and cleared before every test.

    Without this, tests that create users with the same auto-incremented
    primary key could receive stale cached responses from prior tests.
...
def db() -> Generator[Session, None, None]:
    ...
    connection.close()
    # Remove the override so subsequent tests (e.g. contract tests) use the real get_db
